      ...
      try {
        ...
        let query = supabase
          ...
          .select(
            ...
            'id, name, status, last_seen, last_heartbeat_at, registered_at'
          )
        ...
        return {
          devices: resultDevices.map(d => ({
            ...
            lastSeen: d.last_seen ?? d.last_heartbeat_at,
      ...
      try {
        ...
        const { data: device, error } = await supabase
          ...
          .select(
            ...
            'id, name, status, metrics, last_seen, last_heartbeat_at, registered_at, network_info'
          )
          .eq('id', deviceId)
          .eq('customer_id', customerId)
          .single();
        ...
        return {
          device: {
            ...
            lastHeartbeat: device.last_heartbeat_at ?? device.last_seen,
            metrics: (device.metrics as Record<string, unknown>) || undefined,
            networkInfo:
              (device.network_info as Record<string, unknown>) || undefined,
